...
class Player:
    ...
    def enter(self):
        if not self.txtbox.get() or not dic.check(self.txtbox.get()):
            show_error()
            self.txtbox.delete(0, END)
            return
...
def create_desc_label():
    ...
    row = Frame(root)
    lbl_desc = Label(row, text=desc, fg="grey", width=100, height=40, wraplength=250, justify=LEFT)
    lbl_desc.config(font=('Helvetica', '18', 'bold'))
